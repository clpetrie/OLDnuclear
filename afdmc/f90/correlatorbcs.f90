module correlator
   ...
   complex(kind=r8), private, save, allocatable :: sxmallz(:,:,:)
...
!! PP and NN corelations
   ...
   logical, private, save, allocatable :: dofst(:),doft(:),dofs(:),doftpp(:),doftnn(:)
   integer(kind=i4), private, parameter :: levi(2,3) = &
      reshape((/2,3, 3,1, 1,2/),(/2,3/))
   private :: opmult
   complex(kind=r8), private, save, allocatable :: tau(:,:,:),sigma(:,:,:),sigtau(:,:,:,:,:)
   ...
   real(kind=r8), private, save, allocatable :: fsig(:,:,:),ftau(:),fsigtau(:,:,:)
contains
   ...
   if (allocated(sxmallz)) then
      deallocate(sxmallz)
      deallocate(fstvec,fstval,ft)
      deallocate(dofst,doft,dofs,doftpp,doftnn)
      deallocate(tau,sigma,sigtau)
   endif
   allocate(sxmallz(npart,4,npart))
   allocate(fstvec(3,3,npair),fstval(3,npair))
   allocate(fsvec(3,3,npair),fsval(3,npair))
   allocate(ft(npair),dofst(npair),doft(npair),dofs(npair),doftpp(npair),doftnn(npair))
   ...
   allocate(fsig(3,3,npair),ftau(npair),fsigtau(3,3,npair))
   ...
   subroutine calfop(ftauin,fsigin,fsigtauin,ftauppin,ftaunnin,cut)
   use matrixmod
...
!! PP and NN corelations
   ...
   subroutine cordet(detrat,pf,smati,cmatin,sp,nex)
! calfop must be called first
   integer(kind=i4) :: nex
   complex(kind=r8) :: detrat,sp(:,:)
   complex(kind=r8) :: pf(:,:,:,:),smati(:,:)
   integer(kind=i4) :: i,j,ij,is,it,itau,ic1,ic2,kc1,kc2
   complex(kind=r8) :: spx(4,15,npart)
   complex(kind=r8) :: cmatin(npart+nex,15,npart)
   complex(kind=r8) :: cmat(npart+nex,15,npart)
   ...
   complex(kind=r8) :: fac,aij
   spx=opmult(sp)
   
   cmat=cmatin
   pmat=reshape(matmul(smati,reshape(cmat,(/npart+nex,15*npart/))),shape(pmat))
   gmats=reshape(matmul(transpose(reshape(cmat(:,1:3,:),(/npart+nex,3*npart/)))&
      ,reshape(pmat(:,1:3,:),(/npart+nex,3*npart/))),shape(gmats))
   gmatt(:,:,1)=matmul(transpose(cmat(:,4,:)),pmat(:,4,:))
   gmatt(:,:,2)=matmul(transpose(cmat(:,5,:)),pmat(:,5,:))
   gmatt(:,:,3)=matmul(transpose(cmat(:,6,:)),pmat(:,6,:))
   gmatst(:,:,:,:,1)=reshape(matmul(transpose(reshape(cmat(:,7:13:3,:)&
      ,(/npart+nex,3*npart/))),reshape(pmat(:,7:13:3,:),(/npart+nex,3*npart/)))&
      ,shape(gmats))
   gmatst(:,:,:,:,2)=reshape(matmul(transpose(reshape(cmat(:,8:14:3,:)&
      ,(/npart+nex,3*npart/))),reshape(pmat(:,8:14:3,:),(/npart+nex,3*npart/)))&
      ,shape(gmats))
   gmatst(:,:,:,:,3)=reshape(matmul(transpose(reshape(cmat(:,9:15:3,:)&
      ,(/npart+nex,3*npart/))),reshape(pmat(:,9:15:3,:),(/npart+nex,3*npart/)))&
      ,shape(gmats))
   ij=0
   detrat=czero
   do i=1,npart-1
      do j=i+1,npart
         ...
         if (dofst(ij)) then
            do itau=1,3
               do ic1=1,3
                  ...
                  do ic2=1,3
                     kc2=3*ic2+itau+3
                     aij=sum(spx(:,kc1,i)*pf(:,1,i,j))*spx(1,kc2,j) &
                        +sum(spx(:,kc1,i)*pf(:,2,i,j))*spx(2,kc2,j) &
                        +sum(spx(:,kc1,i)*pf(:,3,i,j))*spx(3,kc2,j) &
                        +sum(spx(:,kc1,i)*pf(:,4,i,j))*spx(4,kc2,j)
                     fac=smati(j,i)*(aij+gmatst(ic1,i,ic2,j,itau)) &
                        +pmat(i,kc1,i)*pmat(j,kc2,j)-pmat(j,kc1,i)*pmat(i,kc2,j)
                     detrat=detrat+fac*fsigtau(ic1,ic2,ij)
                  enddo
               enddo
            enddo
         endif
         if (dofs(ij)) then
            do ic1=1,3
               do ic2=1,3
                  aij=sum(spx(:,ic1,i)*pf(:,1,i,j))*spx(1,ic2,j) &
                     +sum(spx(:,ic1,i)*pf(:,2,i,j))*spx(2,ic2,j) &
                     +sum(spx(:,ic1,i)*pf(:,3,i,j))*spx(3,ic2,j) &
                     +sum(spx(:,ic1,i)*pf(:,4,i,j))*spx(4,ic2,j)
                  fac=smati(j,i)*(aij+gmats(ic1,i,ic2,j)) &
                     +pmat(i,ic1,i)*pmat(j,ic2,j)-pmat(j,ic1,i)*pmat(i,ic2,j)
                  detrat=detrat+fac*fsig(ic1,ic2,ij)
               enddo
            enddo
         endif
         if (doft(ij)) then
            do itau=1,3
               aij=sum(spx(:,itau+3,i)*pf(:,1,i,j))*spx(1,itau+3,j) &
                  +sum(spx(:,itau+3,i)*pf(:,2,i,j))*spx(2,itau+3,j) &
                  +sum(spx(:,itau+3,i)*pf(:,3,i,j))*spx(3,itau+3,j) &
                  +sum(spx(:,itau+3,i)*pf(:,4,i,j))*spx(4,itau+3,j)
               fac=smati(j,i)*(aij+gmatt(i,j,itau)) &
                  +pmat(i,itau+3,i)*pmat(j,itau+3,j)-pmat(j,itau+3,i)*pmat(i,itau+3,j)
               detrat=detrat+fac*ftau(ij)
            enddo
         endif
      enddo
   enddo
...
! calfop and cordet must be called first.
   ...
   complex(kind=r8) :: detrat,sxz(4,npart,npart),sxzi(4,npart,npart)
   complex(kind=r8) :: sxzj(4,npart,npart),d1,d2
   ...
   complex(kind=r8) :: sxmallzin(:,:,:)
   sxmallz=sxmallzin
   sxz=reshape(transpose(reshape(sxmallz,(/npart,4*npart/))),shape(sxz))
   d1b=czero
   d2b=czero
   d3b=czero
   call g1bval(d1b,sxz,cone+0.25_r8*fctau)
   call g2bval(d2b,sxz,cone+0.25_r8*fctau)
   call g3bval(d3b,sxz,cone+0.25_r8*fctau)
   do i=1,npart
      sx15(:,:,:,i)=conjg(opmult(conjg(sxz(:,i,:))))
   enddo
   do i=1,npart
      if (abs(ftau1(i)).le.0.0_r8) cycle
      call sxzupdate(sxzi,d1,sxz,i,sx15(:,6,:,i),sp(:,i))
      f1=d1*ftau1(i)
      call g1bval(d1b,sxzi,0.25_r8*f1)
      call g2bval(d2b,sxzi,0.25_r8*f1)
      call g3bval(d3b,sxzi,0.25_r8*f1)
   enddo
   ij=0
   do i=1,npart-1
      do j=i+1,npart
         ...
         do iop=1,15
            ...
            select case (iop)
               case (1:2)
                  if (.not.doft(ij)) cycle
                  fij=ft(ij)
                  call sxzupdate(sxzi,d1,sxz,i,sx15(:,3+iop,:,i),sp(:,i))
               ...
               case (4:12)
                  ...
                  it=iop-3*is
                  fij=fstval(is,ij)
                  op(:,:)=fstvec(1,is,ij)*sx15(:,6+it,:,i) &
                         +fstvec(2,is,ij)*sx15(:,9+it,:,i) &
                         +fstvec(3,is,ij)*sx15(:,12+it,:,i)
                  call sxzupdate(sxzi,d1,sxz,i,op,sp(:,i))
                  sx15j(:,:,:)=conjg(opmult(conjg(sxzi(:,j,:))))
                  op(:,:)=fstvec(1,is,ij)*sx15j(:,6+it,:) &
                         +fstvec(2,is,ij)*sx15j(:,9+it,:) &
                         +fstvec(3,is,ij)*sx15j(:,12+it,:)
...
!
   ...
   call hstnipot(x,vc,xpi,g2s3b,delta,a2p3b,a2s3b,avd,ave,acfac,a2psc,a2pxdsc,a2pddsc,dfac,1.0_r8)
   delsum=0.0_r8
   xd=0.0_r8
   xxpi=0.0_r8
               ...
               do it=1,3
                  do js=1,4
                     ...
                     if (dov3.eq.2) then
                        ...
                        v2tmp3a(:,js)=v2tmp3a(:,js)+xpi(ic,i,jc,j)*( &  ! Kevin's version
                           delsum(j)+delsum(i)-2.0_r8*delta(i,j)) &
                           *spx(:,ic3+it,i)*spx(js,jc3+it,j) ! Vd,1
!                       v2tmp3a(:,js)=v2tmp3a(:,js)+xpi(ic,i,jc,j) &  ! Ingo/Joel's version
!                          *(delsum(j)-delta(i,j))*spx(:,ic3+it,i)*spx(js,jc3+it,j) ! Vd,1
                        if (jc.eq.ic) then
                           v2deldel(:,js)=v2deldel(:,js)+ddelta(i,j) &
                                *spx(:,ic3+it,i)*spx(js,jc3+it,j)  ! Ve,Cc3
                           v2tmp3b(:,js)=v2tmp3b(:,js)+xd(ic,i,ic,j) &  ! Kevin's version
                                *(delsum(j)+delsum(i)-2.0_r8*delta(i,j)) & 
                                *spx(:,ic3+it,i)*spx(js,jc3+it,j) ! Vd,2
!                          v2tmp3b(:,js)=v2tmp3b(:,js)+xd(ic,i,ic,j) &  ! Ingo/Joel's version
!                               *(delsum(j)-delta(i,j))*spx(:,ic3+it,i)*spx(js,jc3+it,j) ! Vd,2
                        endif
                     endif
                  enddo
               enddo
            ...
            if (dov3.eq.2) then
               do js=1,4
                  v2tmp4(:,js)=v2tmp4(:,js)+sum(delta(i,:)*delta(j,:)) &
                         *spx(:,ic+3,i)*spx(js,ic+3,j) ! Ve
               enddo
            endif
         enddo
