...
!! PP and NN corelations
   ...
   logical, private, save, allocatable :: dofst(:),doft(:),dofs(:),doftpp(:),dof(:,:)
...
contains
   ...
   if (allocated(sxz0)) then
      deallocate(sxz0,fsvec,fsval,ft,fs,fst,f15,fstl,fstr,fstval,ftpp,ftnn,ftau1)
      deallocate(dof,dofst,doft,dofs,doftpp,doftnn)
   ...
   subroutine cordet(detrat,sxzin,sp)
! calfop must be called first
   ...
   integer(kind=i4) :: k,l,kl,ks(15),ls,kt,i,j,ij
   complex(kind=r8) :: d1b(4,npart),d2b(4,4,npair),d3b(4,4,4,ntrip)
   complex(kind=r8) :: sx15(4,15,npart,npart),sxzk(4,npart,npart,15),sxzl(4,npart,npart)
   complex(kind=r8) :: d15(15),fkl,sx15l(4,15,npart)
   integer(kind=i4) :: kop,kiter
   sxz0=sxzin
   d1b=czero
   d2b=czero
   d3b=czero
   !Set up dof from dofs, doft, dofst and f15 from fs, ft, fts and ks(15)
   do kiter=1,15
      if (kiter.le.3) then
         dof(kiter,:)=dofs(:)
         do kl=1,3
            do kt=1,3
               f15(kiter,kl,kt,:)=fs(kl,kt,:)
            enddo
         enddo
         ks(kiter)=kiter
      endif
      if (kiter.gt.3 .and. kiter.le.6) then
         dof(kiter,:)=doft(:)
         f15(kiter,1,1,:)=ft(:)
         ks(kiter)=1
      endif
      if (kiter.ge.7) then
         ...
         do kl=1,3
            do kt=1,3
               f15(kiter,kl,kt,:)=fst(kl,kt,:)
            enddo
         enddo
      endif
      if (kiter.eq.7 .or. kiter.eq.8 .or. kiter.eq.9) ks(kiter)=1
      if (kiter.eq.10 .or. kiter.eq.11 .or. kiter.eq.12) ks(kiter)=2
      if (kiter.eq.13 .or. kiter.eq.14 .or. kiter.eq.15) ks(kiter)=3
   enddo
   !Done with set up
...
!Mathematica ij = FullSimplify[Sum[(npart - n), {n, 1, i - 1}] + (j - i)]
         ...
         do kiter=1,15
            if (dof(kiter,kl)) then
               sx15l(:,:,:)=conjg(opmult(conjg(sxzk(:,l,:,kiter))))
               do ls=1,3
                  if (kiter.le.3 .and. ls.eq.2) cycle
                  if (kiter.le.3 .and. ls.eq.3) cycle
                  call sxzupdate(sxzl,d2,sxzk(:,:,:,kiter),l,sx15l(:,kiter,:),sp(:,l))
                  fkl=d15(kiter)*d2*f15(kiter,ks(kiter),ls,kl)
   ...
   integer(kind=i4) :: i,j,ij,iop,is,it,js,k,ks,ijk,iiter,jj(15)
   d1b=czero
   d2b=czero
   d3b=czero
   !Set up jj(15)
   do iiter=1,15
      if (iiter.le.3) jj(iiter)=iiter
      if (iiter.gt.3 .and. iiter.le.6) jj(iiter)=1
      if (iiter.eq.7 .or. iiter.eq.8 .or. iiter.eq.9) jj(iiter)=1
      if (iiter.eq.10 .or. iiter.eq.11 .or. iiter.eq.12) jj(iiter)=2
      if (iiter.eq.13 .or. iiter.eq.14 .or. iiter.eq.15) jj(iiter)=3
   enddo
   !Done with set up
...
!Mathematica ij = FullSimplify[Sum[(npart - n), {n, 1, i - 1}] + (j - i)]
         ...
         do iiter=1,15
            if (dof(iiter,ij)) then
               sx15j(:,:,:)=conjg(opmult(conjg(sxzi(:,j,:,iiter))))
               do js=1,3
                  if (iiter.le.3 .and. js.eq.2) cycle
                  if (iiter.le.3 .and. js.eq.3) cycle
                  call sxzupdate(sxzj,d2,sxzi(:,:,:,iiter),j,sx15j(:,iiter,:),sp(:,j))
                  fij=d15(iiter)*d2*f15(iiter,jj(iiter),js,ij)
                  if (iiter.eq.3 .and. doftpp(ij)) fij=fij+0.25_r8*ftpp(ij)
                  if (iiter.eq.3 .and. doftnn(ij)) fij=fij+0.25_r8*ftnn(ij)
