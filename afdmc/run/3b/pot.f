!$Id: pot.f,v 1.1.1.1 2013/08/26 20:57:28 nuclear Exp $
c *id* block data ******************************************************
c identifies potentials that have no terms quadratic in L or p
c ----------------------------------------------------------------------
      block data lpots
      logical lpotls,lpotll
      common /logpot/ lpotls(24),lpotll(24)
      data lpotls/.false.,.true.,.true.,.true.,.true.,.true.,.true.
     &           ,.true.,.true.,.true.,.false.,.false.,.false.,.false.
     &           ,.false.,.true.,.true.,.true.,.true.,.true.
     &           ,.true.,.true.,.true.,.true./
      data lpotll/.false.,.false.,.true.,.false.,.true.,.true.,.true.
...
c and obtain values for hbar**2/m.
...
c        14=argonne v1'
c        15=argonne vx'
c        16=argonne v18p  (p**2 terms)
c        17=argonne v18pq (p**2 terms & l**2 tensor)
c        18=super-soft core(c) v14
c        19=super-soft core(c) v8' modified
c        20=paris
...
c vv:   potential in MeV (18 component array)
c vp:   p**2 potential terms (12 component array)
c vw:   subsidiary potential terms in MeV (14 component array)
c ----------------------------------------------------------------------
...
c       6=OPEP (s1xs2).L(t1xt2)z         7=OREP (s1-s2).L(t1-t2)z
c       8=OREP (s1xs2).L(t1xt2)z         9=ORMX (s1-s2).L(t1-t2)z
c ----------------------------------------------------------------------
...
c -------------
      ...
      if ((lpot.ge.7 .and. lpot.le.17) .or. lpot.ge.21) then
...
c ------------------------------
      if ((lpot.ge.4 .and. lpot.le.17) .or. lpot.ge.21) then
        pimass1=gam
        pimass2=rho
        pimass3=chi
        wsrange=1.-(2./3.)*(omg-1.)
...
c -----------
      if(v7cut) then
c              adjust p10 to get deuteron correct
!             p10=p10-0.30*pls0+0.181*pls1
!!!! Joe's new potential (crazy)
!     p10=p10-1.94260*tpi2+2662.52*ws-1613.158*wsx2
!     pt0=pt0+2*(-2.060275*tpi2-843.80386*wsx+2420.9726*wsx2)
! second version
      p10=p10+1.18522*tpi2+3125.460*wsx-3961.561*wsx2
      pt0=pt0+2*(0.062339*tpi2-1799.755*wsx+1713.649*wsx2)
!!!!
      endif
      vv(1)=.0625*(9*p11+3*p10+3*p01+p00)
      vv(2)=.0625*(3*p11-3*p10  +p01-p00)
...
c -----------------------
        ...
        vw(7)=2*fmsqi*(1+komg)*(1+krho)*wspds/3
        vw(8)=-fmsqi*(1+komg)*(1+krho)*(wspds-3*wspp/rr)/3
        vw(9)=4*fmsqi*(2+komg+krho)*wspp/rr
        vw(10)=4*fmsqi*(krho-komg)*wspp/rr
        mrho=770.
        murho=mrho/hc
        xrho=murho*rr
        rcut=1-exp(-cpi*rr*rr)
        ...
        vw(11)=2*fsq*deltam*zpic
        vw(12)=2*grho*deltam*(mrho/(mn+mp))**2*zrho
        vw(13)=2*(1+krho)**2*vw(12)
        end if
      end if
